#!/usr/bin/env bash
# Scrape real intel, start server, seed it, ready for nur report
set -e
cd "$(dirname "$0")/.."
...
echo "  Starting nur server..."
rm -f demo_seeded.db
nur serve --port 8000 --db sqlite+aiosqlite:///demo_seeded.db > /dev/null 2>&1 &
SERVER_PID=$!
echo $SERVER_PID > /tmp/nur_server.pid
sleep 2

echo "  Uploading all seed data..."
count=0
for f in demo/seed/ioc_bundle_*.json demo/seed/attack_map_*.json; do
    nur upload "$f" --api-url http://localhost:8000 --yes > /dev/null 2>&1
    count=$((count + 1))
done
...
echo ""
echo "  Try:"
echo "    nur report demo/ioc_bundle_2.json --api-url http://localhost:8000"
echo "    nur report demo/attack_map_lockbit.json --api-url http://localhost:8000"
echo "    nur report demo/eval_crowdstrike.json --api-url http://localhost:8000"
echo ""
echo "  Stop server: kill \$(cat /tmp/nur_server.pid)"
