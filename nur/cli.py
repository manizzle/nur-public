...
# ── Federated Learning (research — not loaded in production) ─────────────────
# These commands are only registered when NUR_ENABLE_FL=1 or the FL
# dependencies are explicitly installed.  This keeps the default CLI
# lean and avoids loading numpy/torch on every invocation.
...
# ── Graph Intelligence (research — not loaded in production) ─────────────────
# Same as FL: only registered when NUR_ENABLE_GRAPH=1 to keep the default
# CLI lean and avoid loading numpy/networkx/torch-geometric.

if os.environ.get("NUR_ENABLE_GRAPH") == "1":
...
# graph analyze and compare commands — guarded under NUR_ENABLE_GRAPH above.
# They reference the `graph` click group which is only defined when the flag is set.
if os.environ.get("NUR_ENABLE_GRAPH") == "1":
